...
*   **[DONE/PARTIAL] Remove Cons-Packing from Built-ins**: Hot arithmetic argument materialization now keeps constants, locals, and promotable expressions in SSA where possible. `gethash`, `puthash`, `(setf (gethash ...))`, and dynamic `(setf gethash)` calls lower to direct fixed-arity runtime calls instead of generic stack funcall/Cons argument packing. Rank-1 `aref` and `(setf aref)` can lower through raw-index fixed-arity entry points in AOT when the index is a proven or locally unboxable fixnum. AOT comparisons of the shape `(= (aref vector i) <fixnum>)` now use a direct raw-index equality helper instead of materializing the element and expected value through the generic numeric comparison path. Remaining work: rewrite `compile_stack_builtin_call` and the remaining non-arithmetic built-ins to pass fixed arguments instead of constructing heap `Cons` lists.
...
*   **[DONE/PARTIAL] SSA Stack Promotion**: `compile_expr_as_ssa` now avoids push/pop round trips for constants, locals, user-call arguments, hot arithmetic operands, `setq`, `let`, `let*`, `gethash`/`puthash`, and `dotimes` counts. Simple fixed-arity compiled functions are registered for AOT inlining from both defun emission paths. Loop-carried fixnum variables can now stay in raw unboxed SSA for hot `dotimes` bodies when assignments preserve raw fixnum values; this covers the current `function_call_hotloop` LCG path. Parsed simple `loop` iteration variables are also exposed as raw fixnum SSA inside the loop body, allowing array index operations such as sieve-style `(aref flags i)` to bypass boxed index materialization. The compiler also has a direct raw Euclidean GCD lowering for the benchmarked two-argument loop shape while preserving correct `return-from` handling. Remaining work: generalize the raw loop-carried analysis beyond the current recognized hot shapes and reduce residual statement-position stack traffic.
