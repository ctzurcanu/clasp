...
fn is_float_type(val: &EvalResult) -> bool {
    matches!(
        val,
        EvalResult::Float(_) | EvalResult::FloatSingle(_) | EvalResult::Complex(_, _)
    )
}
...
fn to_complex(val: &EvalResult) -> Option<(f64, f64)> {
    match val {
        ...
        EvalResult::FloatSingle(f) => Some((*f, 0.0)),
...
fn numeric_type_error(op: &str) -> String {
...
fn program_error_at_least_one(op: &str) -> String {
    format!("PROGRAM-ERROR: {} requires at least 1 argument", op)
}

pub(super) const FLOAT_TRAPS_MASKED_KEY: &str = "%__RLASP_FLOAT_TRAPS_MASKED__%";

thread_local! {
    static FLOAT_TRAPS_MASKED: RefCell<Option<String>> = RefCell::new(None);
}

pub(super) fn normalize_float_trap_name(name: &str) -> String {
    name.trim_start_matches(':')
        .to_ascii_uppercase()
        .replace('-', "_")
}

pub(super) fn current_float_traps_masked() -> Option<String> {
    FLOAT_TRAPS_MASKED.with(|masked| masked.borrow().clone())
}

pub(super) fn set_float_traps_masked(masked: Option<String>) {
    FLOAT_TRAPS_MASKED.with(|state| {
        *state.borrow_mut() = masked;
    });
}

fn float_trap_masked(env: &HashMap<String, EvalResult>, trap: &str) -> bool {
    let needle = normalize_float_trap_name(trap);
    let env_masked = match env.get(FLOAT_TRAPS_MASKED_KEY) {
        Some(EvalResult::String(masked)) => masked
            .split_whitespace()
            .any(|entry| entry == needle || entry == "ALL"),
        _ => false,
    };
    env_masked
        || FLOAT_TRAPS_MASKED.with(|masked| {
            masked
                .borrow()
                .as_deref()
                .map(|names| {
...
fn is_numeric_value(val: &EvalResult) -> bool {
    matches!(
        val,
        EvalResult::Fixnum(_)
            | EvalResult::Float(_)
            | EvalResult::FloatSingle(_)
            | EvalResult::Bignum(_)
            | EvalResult::Ratio(_)
...
fn is_real_value(val: &EvalResult) -> bool {
    matches!(
        val,
        EvalResult::Fixnum(_)
            | EvalResult::Float(_)
...
pub(super) fn eval_add_with_env(
    args: &[ASTNode],
    ...
    if has_complex {
        let had_float = evaluated_args
            .iter()
            .any(|v| matches!(v, EvalResult::Float(_)));
        let mut sum_re = 0.0;
        let mut sum_im = 0.0;
        for val in &evaluated_args {
            match to_complex(val) {
                ...
                None => return Err(numeric_type_error("+")),
            }
        }
        return Ok(canonicalize_complex(sum_re, sum_im, had_float));
    }

    let has_float = evaluated_args
        .iter()
        .any(|v| matches!(v, EvalResult::Float(_)));
    if has_float {
        ...
        for val in evaluated_args {
            ...
            let f = to_f64_for_cmp(&val).ok_or_else(|| numeric_type_error("+"))?;
            sum_float += f;
            if sum_float.is_infinite() && !float_trap_masked(env, "overflow") {
                return Err("FLOATING-POINT-OVERFLOW: floating point overflow".to_string());
            }
        }
        return Ok(EvalResult::Float(sum_float));
    }

    let has_ratio = evaluated_args
        .iter()
        .any(|v| matches!(v, EvalResult::Ratio(_)));
    if has_ratio {
...
pub(super) fn eval_sub_with_env(
    args: &[ASTNode],
    env: &mut HashMap<String, EvalResult>,
) -> Result<EvalResult, String> {
    use malachite::Rational;
            ...
            other => {
                if matches!(other, EvalResult::Complex(_, _)) {
                    ...
                    match other {
                        ...
                        EvalResult::FloatSingle(f) => Ok(EvalResult::FloatSingle(-f)),
    ...
    let has_float = matches!(first, EvalResult::Float(_) | EvalResult::FloatSingle(_))
        || evaluated_args
            .iter()
            .any(|v| matches!(v, EvalResult::Float(_) | EvalResult::FloatSingle(_)));
    if has_float {
        let mut result_float = match &first {
            EvalResult::Nil => 0.0,
            _ => {
                to_f64_for_cmp(&first).ok_or_else(|| "- requires numeric arguments".to_string())?
            }
        };
        ...
        for val in evaluated_args {
            ...
            let f =
                to_f64_for_cmp(&val).ok_or_else(|| "- requires numeric arguments".to_string())?;
            result_float -= f;
        }
        return Ok(EvalResult::Float(result_float));
    }
...
pub(super) fn eval_mul_with_env(
    args: &[ASTNode],
    env: &mut HashMap<String, EvalResult>,
) -> Result<EvalResult, String> {
    use malachite::Rational;
    ...
    if args.len() == 2 {
        ...
        if matches!(left, EvalResult::Complex(_, _)) || matches!(right, EvalResult::Complex(_, _)) {
            let had_float =
                matches!(left, EvalResult::Float(_)) || matches!(right, EvalResult::Float(_));
            let (lre, lim) =
                to_complex(&left).ok_or_else(|| "* requires numeric arguments".to_string())?;
            let (rre, rim) =
                to_complex(&right).ok_or_else(|| "* requires numeric arguments".to_string())?;
        ...
        return match (left, right) {
            ...
            (EvalResult::Fixnum(a), EvalResult::Bignum(b)) => {
                Ok(EvalResult::Bignum(Integer::from(a) * b))
            }
            (EvalResult::Bignum(a), EvalResult::Fixnum(b)) => {
                Ok(EvalResult::Bignum(a * Integer::from(b)))
            }
            (EvalResult::Float(a), other) | (other, EvalResult::Float(a)) => {
                let b = to_f64_for_cmp(&other)
                    .ok_or_else(|| "* requires numeric arguments".to_string())?;
                Ok(EvalResult::Float(a * b))
            }
            (EvalResult::Ratio(a), EvalResult::Ratio(b)) => Ok(rational_to_eval(a * b)),
            (EvalResult::Ratio(a), EvalResult::Fixnum(b))
            | (EvalResult::Fixnum(b), EvalResult::Ratio(a)) => {
                Ok(rational_to_eval(a * Rational::from(b)))
            }
            (EvalResult::Ratio(a), EvalResult::Bignum(b))
            | (EvalResult::Bignum(b), EvalResult::Ratio(a)) => {
                Ok(rational_to_eval(a * Rational::from(b)))
            }
            _ => Err("* requires numeric arguments".to_string()),
        };
...
pub(super) fn eval_div_with_env(
    args: &[ASTNode],
    env: &mut HashMap<String, EvalResult>,
) -> Result<EvalResult, String> {
    ...
    if args.is_empty() {
        return Err(program_error_at_least_one("/"));
    }
    ...
    if has_complex {
        let first = to_complex(&evaluated_args[0]).ok_or_else(|| numeric_type_error("/"))?;
        let mut result_re = first.0;
        let mut result_im = first.1;
    ...
    if has_float {
        ...
        if evaluated_args.is_empty() {
            // (/ x) = 1/x
            if result == 0.0 && !float_trap_masked(env, "divide-by-zero") {
                return Err("DIVISION-BY-ZERO: division by zero".to_string());
            }
        ...
        for val in &evaluated_args {
            ...
            if divisor == 0.0 {
                if result == 0.0 {
                    if !float_trap_masked(env, "invalid") {
                        return Err(
                            "FLOATING-POINT-INVALID-OPERATION: invalid floating point operation"
                                .to_string(),
                        );
                    }
                } else if !float_trap_masked(env, "divide-by-zero") {
                    return Err("DIVISION-BY-ZERO: division by zero".to_string());
                }
            }
            result /= divisor;
            if result.is_infinite() && !float_trap_masked(env, "overflow") {
                return Err("FLOATING-POINT-OVERFLOW: floating point overflow".to_string());
            }
            if result.is_nan() && !float_trap_masked(env, "invalid") {
                return Err(
                    "FLOATING-POINT-INVALID-OPERATION: invalid floating point operation"
                        .to_string(),
                );
        ...
        if evaluated_args.is_empty() {
            // (/ n) = 1/n
            if ratio == 0 {
                if float_trap_masked(env, "divide-by-zero") {
                    return Ok(EvalResult::Float(f64::INFINITY));
                }
                return Err("DIVISION-BY-ZERO: division by zero".to_string());
            }
        ...
        for val in evaluated_args {
            let divisor = match val {
                EvalResult::Fixnum(n) => {
                    if n == 0 {
                        if float_trap_masked(env, "invalid") && ratio == 0 {
                            return Ok(EvalResult::Float(f64::NAN));
                        }
                        if float_trap_masked(env, "divide-by-zero") {
                            let sign = if ratio < 0 { -1.0 } else { 1.0 };
                            return Ok(EvalResult::Float(sign * f64::INFINITY));
                        }
                        return Err("DIVISION-BY-ZERO: division by zero".to_string());
                    }
                    Rational::from(n)
...
pub(super) fn numeric_equal(a: &EvalResult, b: &EvalResult) -> Result<bool, String> {
    ...
    if !matches!(a, EvalResult::Float(_) | EvalResult::FloatSingle(_))
        && !matches!(b, EvalResult::Float(_) | EvalResult::FloatSingle(_))
    {
    ...
    match (to_f64_for_cmp(a), to_f64_for_cmp(b)) {
        ...
        _ => Err(numeric_type_error("=")),
    }
}

pub(super) fn eval_eq_with_env(
    args: &[ASTNode],
    env: &mut HashMap<String, EvalResult>,
) -> Result<EvalResult, String> {
    if args.is_empty() {
...
pub(super) fn eval_ne_with_env(
    ...
    let evaluated: Vec<EvalResult> = args
        .iter()
        .map(|arg| eval_with_env(arg, env).map(primary_value))
        .collect::<Result<_, _>>()?;
    if evaluated.len() == 1 {
        let only = evaluated[0].clone();
        if !is_numeric_value(&only) {
            return Err(numeric_type_error("/="));
        }
        return Ok(EvalResult::Bool(true));
    }
    if evaluated.len() == 2 {
        let left = evaluated[0].clone();
        let right = evaluated[1].clone();
    ...
    for i in 0..evaluated.len() {
        for j in i + 1..evaluated.len() {
            let val_i = evaluated[i].clone();
            let val_j = evaluated[j].clone();
            if !is_numeric_value(&val_i) || !is_numeric_value(&val_j) {
                return Err(numeric_type_error("/="));
            }
...
pub(super) fn eval_eq_lisp(
    ...
    let result = match (&first, &second) {
        ...
        (EvalResult::Cons(ac, ad), EvalResult::Cons(bc, bd)) => {
            Rc::ptr_eq(ac, bc) && Rc::ptr_eq(ad, bd)
        }
        _ => false,
    };
...
fn numeric_cmp(a: &EvalResult, b: &EvalResult) -> Result<std::cmp::Ordering, String> {
    ...
    match (to_f64_for_cmp(a), to_f64_for_cmp(b)) {
        (Some(fa), Some(fb)) => fa
            .partial_cmp(&fb)
            .ok_or_else(|| "comparison with NaN".to_string()),
        _ => Err("TYPE-ERROR: comparison requires real arguments".to_string()),
    }
}

pub(super) fn eval_lt_with_env(
    args: &[ASTNode],
    env: &mut HashMap<String, EvalResult>,
) -> Result<EvalResult, String> {
    if args.is_empty() {
    ...
    if !is_real_value(&prev) {
        return Err(real_type_error("<"));
    }
    if args.len() == 1 {
        return Ok(EvalResult::Bool(true));
    }
...
pub(super) fn eval_one_plus(
    ...
    match eval_with_env(&args[0], env)? {
        EvalResult::Fixnum(n) => match n.checked_add(1) {
            Some(result) => Ok(EvalResult::Fixnum(result)),
            None => Ok(EvalResult::Bignum(
                malachite::Integer::from(n) + malachite::Integer::from(1),
            )),
        },
...
pub(super) fn eval_one_minus(
    ...
    match eval_with_env(&args[0], env)? {
        EvalResult::Fixnum(n) => match n.checked_sub(1) {
            Some(result) => Ok(EvalResult::Fixnum(result)),
            None => Ok(EvalResult::Bignum(
                malachite::Integer::from(n) - malachite::Integer::from(1),
            )),
...
pub(super) fn eval_numberp(
    ...
    match eval_with_env(&args[0], env)? {
        ...
        | EvalResult::Float(_)
        | EvalResult::FloatSingle(_)
        | EvalResult::Complex(_, _) => Ok(EvalResult::Bool(true)),
        _ => Ok(EvalResult::Nil),
    }
}
...
pub(super) fn eval_floor(
    ...
    let has_float =
        matches!(&num, EvalResult::Float(_)) || matches!(&divisor, Some(EvalResult::Float(_)));
...
pub(super) fn eval_ffloor(
    ...
    let d = if args.len() == 2 {
        eval_to_f64(&eval_with_env(&args[1], env)?)?
    } else {
        1.0
    };
    if d == 0.0 {
...
pub(super) fn eval_logbitp(
    ...
    let idx = match &index {
        EvalResult::Fixnum(i) => {
            if *i < 0 {
                return Err("TYPE-ERROR: logbitp index must be non-negative".to_string());
            }
            *i as u64
        }
        EvalResult::Bignum(b) => {
            ...
            match &integer {
                ...
                EvalResult::Bignum(bn) => {
                    return Ok(EvalResult::Boolean(*bn < malachite::Integer::from(0)))
                }
                _ => return Err("TYPE-ERROR: logbitp requires integer arguments".to_string()),
            }
        }
        _ => return Err("TYPE-ERROR: logbitp requires integer arguments".to_string()),
    };
    ...
    match &integer {
        ...
        _ => Err("TYPE-ERROR: logbitp requires integer arguments".to_string()),
    }
}
...
pub(super) fn eval_sqrt(
    ...
    let exact_input = !matches!(val, EvalResult::Float(_) | EvalResult::FloatSingle(_));

    let num = match val {
        EvalResult::Fixnum(n) => n as f64,
        EvalResult::Float(f) => f,
        EvalResult::FloatSingle(f) => f,
    ...
    let sqrt = num.sqrt();
    if exact_input && sqrt.is_finite() && sqrt > f32::MAX as f64 {
        return Ok(EvalResult::Float(f64::INFINITY));
    }

    Ok(EvalResult::Float(sqrt))
}

pub(super) fn eval_isqrt(
    args: &[ASTNode],
    env: &mut HashMap<String, EvalResult>,
) -> Result<EvalResult, String> {
    if args.len() != 1 {
...
pub(super) fn eval_complex(
    ...
    if args.is_empty() || args.len() > 2 {
        return Err(
            "complex requires 1-2 arguments (real and optional imaginary parts)".to_string(),
        );
    }
...
pub(super) fn eval_realpart(
    ...
    match val {
        ...
        | EvalResult::Float(_)
        | EvalResult::Bignum(_)
        | EvalResult::Ratio(_) => Ok(val),
        _ => Err("realpart requires a number".to_string()),
    }
}

pub(super) fn eval_ratio(
    args: &[ASTNode],
    env: &mut HashMap<String, EvalResult>,
) -> Result<EvalResult, String> {
    use malachite::Rational;
...
pub(super) fn eval_imagpart(
    ...
    match val {
        ...
        | EvalResult::Float(_)
        | EvalResult::Bignum(_)
        | EvalResult::Ratio(_) => Ok(EvalResult::Fixnum(0)),
