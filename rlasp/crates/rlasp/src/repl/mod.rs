...
impl Repl {
    ...
    pub fn eval(&mut self, input: &str) -> Result<EvalResult, String> {
        // 1. Read using proper rlasp-reader
        let lisp_obj = match rlasp_reader::read_from_string(input) {
            Ok(obj) => obj,
            Err(e) => {
                // Handle comment-only lines gracefully
                let err_msg = format!("{}", e);
                if err_msg.contains("Unexpected end of input") && input.trim_start().starts_with(';') {
        ...
        // 2. Expand macros
        let expanded = self.expander.macroexpand(lisp_obj)
    ...
    pub fn eval_file(&mut self, input: &str) -> Result<EvalResult, String> {
        ...
        let lisp_objs = rlasp_reader::read_all_from_string(input)
            .map_err(|e| format!("Read error: {}", e))?;

        let mut last_result = EvalResult::Nil;

        // Evaluate each form in sequence
    ...
    fn try_eval_ffi(&mut self, ast: &crate::ir::ASTNode) -> Result<Option<EvalResult>, String> {
            ...
            if let ASTNode::Variable(name) = function.as_ref() {
                match name.as_str() {
                    "load-lib" => {
                        ...
                        if let ASTNode::Constant(crate::ir::ConstantValue::String(lib_name)) = &args[0] {
                    ...
                    "defforeign" => {
                        ...
                        let func_name = if let ASTNode::Variable(n) = &args[0] { n.clone() } else {
                            return Err("Function name must be a symbol".to_string());
                        };

                        let symbol_name = if let ASTNode::Constant(crate::ir::ConstantValue::String(s)) = &args[1] {
                            s.clone()
                        } else {
                        ...
                        for (lib_name, lib) in &self.libraries {
                            if let Ok(func) = lib.get_function(&symbol_name, signature.clone()) {
                                ...
                                return Ok(Some(EvalResult::Symbol(format!("Defined {}", func_name))));
                            }
                        }
                        return Err(format!("Symbol '{}' not found in any loaded library", symbol_name));
                    }
                    _ => {
                        ...
                        if name.contains(':') {
                            ...
                            if parts.len() == 2 {
                                ...
                                if let Some(lib) = self.libraries.get(lib_name) {
                                    ...
                                    let param_types = vec![rlasp_ffi::ForeignType::Double; args.len()];
    ...
    fn parse_type_list(&self, ast: &crate::ir::ASTNode) -> Result<Vec<rlasp_ffi::ForeignType>, String> {
        use crate::ir::ASTNode;
        ...
        loop {
            ...
            match io::stdin().read_line(&mut line) {
                Ok(0) => {
                    ...
                    if !accumulated_input.trim().is_empty() && !self.is_only_comments(&accumulated_input) {
