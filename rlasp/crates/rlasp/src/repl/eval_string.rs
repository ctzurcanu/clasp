...
pub fn register_string_builtins(env: &mut HashMap<String, EvalResult>) {
    ...
    env.insert(
        "stringp".to_string(),
        EvalResult::BuiltinFunction("stringp".to_string()),
    );
    env.insert(
        "simple-string-p".to_string(),
        EvalResult::BuiltinFunction("simple-string-p".to_string()),
    );

    // String construction
    env.insert(
        "make-string".to_string(),
        EvalResult::BuiltinFunction("make-string".to_string()),
    );
    env.insert(
        "string".to_string(),
    ...
    env.insert(
        ...
        EvalResult::BuiltinFunction("string-equal".to_string()),
    );
    env.insert(
        "string-not-equal".to_string(),
        EvalResult::BuiltinFunction("string-not-equal".to_string()),
    );
    env.insert(
        "string-lessp".to_string(),
        EvalResult::BuiltinFunction("string-lessp".to_string()),
    );
    env.insert(
        "string-greaterp".to_string(),
        EvalResult::BuiltinFunction("string-greaterp".to_string()),
    );
    env.insert(
        "string-not-greaterp".to_string(),
        EvalResult::BuiltinFunction("string-not-greaterp".to_string()),
    );
    env.insert(
        "string-not-lessp".to_string(),
        EvalResult::BuiltinFunction("string-not-lessp".to_string()),
    );

    // String case conversion
    env.insert(
        "string-upcase".to_string(),
        EvalResult::BuiltinFunction("string-upcase".to_string()),
    );
    env.insert(
        "string-downcase".to_string(),
        EvalResult::BuiltinFunction("string-downcase".to_string()),
    );
    env.insert(
        "string-capitalize".to_string(),
        EvalResult::BuiltinFunction("string-capitalize".to_string()),
    );
    env.insert(
        "nstring-upcase".to_string(),
        EvalResult::BuiltinFunction("nstring-upcase".to_string()),
    );
    env.insert(
        "nstring-downcase".to_string(),
        EvalResult::BuiltinFunction("nstring-downcase".to_string()),
    );
    env.insert(
        "nstring-capitalize".to_string(),
        EvalResult::BuiltinFunction("nstring-capitalize".to_string()),
    );

    // String trimming
    env.insert(
        "string-trim".to_string(),
        EvalResult::BuiltinFunction("string-trim".to_string()),
    );
    env.insert(
        "string-left-trim".to_string(),
        EvalResult::BuiltinFunction("string-left-trim".to_string()),
    );
    env.insert(
        "string-right-trim".to_string(),
        EvalResult::BuiltinFunction("string-right-trim".to_string()),
    );
}
...
pub fn call_string_builtin(name: &str, args: &[EvalResult]) -> Result<EvalResult, String> {
    ...
    match name.as_str() {
        "stringp" => Ok(EvalResult::Boolean(matches!(
            args.get(0),
            Some(EvalResult::String(_) | EvalResult::Array(_))
        ))),

        "simple-string-p" => Ok(EvalResult::Boolean(matches!(
            args.get(0),
            Some(EvalResult::String(_) | EvalResult::Array(_))
        ))),
        ...
        | "string-not-lessp" => {
            ...
            let mut end1 = a_chars.len();
            let mut start2 = 0usize;
            let mut end2 = b_chars.len();
            let mut i = 2;
            while i + 1 < args.len() {
            ...
            let result = match name.as_str() {
                "string=" | "string-equal" => {
                    if mismatch_pos.is_none() {
                        EvalResult::Bool(true)
                    } else {
                        EvalResult::Nil
                    }
                }
                "string/=" | "string-not-equal" => match mismatch_pos {
                ...
                "string<" | "string-lessp" => {
                    match mismatch_pos {
                        Some(pos) => {
                            ...
                            if less {
                                EvalResult::Fixnum((start1 + pos) as i64)
                            } else {
                                EvalResult::Nil
                            }
                        }
                ...
                "string>=" | "string-not-lessp" => match mismatch_pos {
        ...
        "string-left-trim" => {
            ...
            match args.get(1) {
                ...
                _ => Err(
                    "string-left-trim requires a character bag and string designator".to_string(),
                ),
            }
        }
        ...
        "string-right-trim" => {
            ...
            match args.get(1) {
                ...
                _ => Err(
                    "string-right-trim requires a character bag and string designator".to_string(),
                ),
            }
        }
        ...
        "char" => match (args.get(0), args.get(1)) {
            (Some(EvalResult::String(s)), Some(EvalResult::Fixnum(idx))) if *idx >= 0 => s
                .chars()
                .nth(*idx as usize)
                .map(EvalResult::Character)
                .ok_or_else(|| "Index out of bounds".to_string()),
            (Some(EvalResult::String(s)), Some(EvalResult::Float(idx))) if *idx >= 0.0 => s
                .chars()
                .nth(*idx as usize)
                .map(EvalResult::Character)
                .ok_or_else(|| "Index out of bounds".to_string()),
