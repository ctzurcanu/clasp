...
fn eval_defstruct_runtime(args: &[ASTNode], env: &mut HashMap<String, EvalResult>) -> Result<EvalResult, String> {
    ...
    let mut struct_name = ast_symbol_name(&args[0])
    ...
    if let Some(parent_name) = include_parent.as_ref() {
        ...
        if let Some(parent_def) = parent_def {
            ...
            if conc_name.is_none() {
                conc_name = Some(parent_def.conc_name.clone());
            }
        }
    }
    ...
    match &expanded {
        ...
        ASTNode::Variable(name) => {
            ...
            match lookup_name_lower.as_str() {
                ...
                "*trace-output*" => Ok(EvalResult::Symbol("*trace-output*".to_string())),
                "*debug-io*" => Ok(EvalResult::Symbol("*debug-io*".to_string())),
        ...
        ASTNode::Setq { var, value } => {
            ...
            // Also update global dynamic store for special variables (*earmuffs*)
            ...
            for slot in slots {
                ...
                if let Some(ref accessor_name) = slot.accessor {
                    ...
                    if accessor_base != accessor_name {
                        env.insert(format!("{}{}", FUNCTION_NS_PREFIX, accessor_base), getter.clone());
                    }
                ...
                if let Some(ref reader_name) = slot.reader {
                    ...
                    if reader_base != reader_name {
                        env.insert(format!("{}{}", FUNCTION_NS_PREFIX, reader_base), getter.clone());
                    }
                }
                ...
                match mode {
                    ...
                    "key" => {
                        if let Ok(spec) = parse_key_param_spec(&arg) {
                            params.push(spec.var_name.clone());
                            key_params.insert(spec.var_name.clone(), spec.key_name);
                            if let Some(def) = spec.default {
                                defaults.insert(spec.var_name.clone(), def);
                            }
                            if let Some(supplied) = spec.supplied_p {
                                supplied_p_vars.insert(spec.var_name.clone(), supplied);
                            }
                        }
                    }
                    "aux" => {
...
pub(in crate::repl) fn eval_call_with_env(function: &ASTNode, args: &[ASTNode], env: &mut HashMap<String, EvalResult>) -> Result<EvalResult, String> {
    if let ASTNode::Variable(name) = function {
        ...
        match base_name {
            ...
            "source-location" if name.starts_with("ext:") || name.starts_with("EXT:") => {
            ...
            "source-location-p" if name.starts_with("ext:") || name.starts_with("EXT:") => {
                if args.len() != 1 {
            ...
            "external-process-wait" if name.starts_with("ext:") || name.starts_with("EXT:") => {
                if args.is_empty() {
            ...
            "external-process-error-stream" if name.starts_with("ext:") || name.starts_with("EXT:") => {
                if args.is_empty() {
            ...
            "read-from-string" => {
                ...
                while i + 1 < args.len() {
                    ...
                    match key.as_str() {
                        ...
                        "end" => end = Some(parse_nonneg(val, "end")?),
                        "preserve-whitespace" => {
            ...
            _ => {
                ...
                if let Ok(eval_args) = eval_args {
                    ...
                    match super::eval_package::call_package_builtin(base_name, &eval_args) {
                        Ok(result) => return Ok(result),
                        Err(e) if !is_unknown_builtin(&e) => return Err(e),
                        _ => {}
                    }
...
fn eval_tail_position(
    ...
    match expanded {
        ...
        ASTNode::Block { name, body } => {
            let block_name = name.unwrap_or_else(|| "nil".to_string());
            if body.is_empty() {
                return Ok(TailEvalResult::Value(EvalResult::Nil));
            }
    ...
    loop {
        ...
        let rest_param = rest_pos.and_then(|r| {
            ...
            if next < current_params.len()
                && current_params.get(next).map_or(false, |p| p != "&aux" && p != "&key")
            {
...
fn parse_key_param_spec(param: &ASTNode) -> Result<KeyParamSpec, String> {
    ...
    match param {
        ...
        ASTNode::Call { function, args } => match &**function {
            ASTNode::Variable(var) => {
                // Handle (:key var) shorthand (no default)
                if var.starts_with(':') && args.len() == 1 {
                    if let Some(var_name) = extract_var_name(&args[0]) {
            ...
            ASTNode::Constant(ConstantValue::Symbol(var)) => {
                ...
                // Handle (:key var default [supplied-p]) form
                if var.starts_with(':') {
                    if let Some(var_name) = args.get(0).and_then(extract_var_name) {
                        return Ok(KeyParamSpec {
                            key_name: trim_keyword(var),
                            var_name,
                            default: args.get(1).cloned(),
                            supplied_p: args.get(2).and_then(extract_var_name),
                        });
                    }
                }
                Ok(KeyParamSpec {
            ...
            ASTNode::Call { function: key_fn, args: key_args } => {
                if let Some(key_name) = extract_name(&**key_fn) {
                    if let Some(var) = key_args.first().and_then(extract_var_name) {
                        return Ok(KeyParamSpec {
