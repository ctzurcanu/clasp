...
pub fn call_env_builtin(name: &str, args: &[EvalResult]) -> Result<EvalResult, String> {
    match name {
        "sleep" => match args.get(0) {
            ...
            _ => Err("TYPE-ERROR: sleep requires a non-negative number".to_string()),
        },
        ...
        "get-internal-run-time" => {
            ...
            match SystemTime::now().duration_since(UNIX_EPOCH) {
                Ok(duration) => Ok(EvalResult::Fixnum(duration.as_millis() as i64)),
                Err(_) => Err("Failed to get system time".to_string()),
            }
        }

        "lisp-implementation-type" => Ok(EvalResult::String("rlasp".to_string())),

        "lisp-implementation-version" => Ok(EvalResult::String("0.1.0".to_string())),

        "machine-type" => Ok(EvalResult::String(std::env::consts::ARCH.to_string())),

        "machine-version" => Ok(EvalResult::String("unknown".to_string())),

        "machine-instance" => Ok(EvalResult::String("localhost".to_string())),

        "software-type" => Ok(EvalResult::String(std::env::consts::OS.to_string())),

        "software-version" => Ok(EvalResult::String("unknown".to_string())),

        "short-site-name" | "long-site-name" => Ok(EvalResult::String("unknown".to_string())),
